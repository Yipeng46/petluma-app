import { HeroProductVisual } from "./HeroProductVisual";
import { RegistryButton } from "./ui";

export function HeroSection() {
  return (
    <section className="relative min-h-[calc(100svh-4.25rem)] overflow-hidden md:min-h-[calc(100svh-4.75rem)]">
      <div className="mx-auto flex h-full min-h-[inherit] max-w-[1400px] flex-col px-6 py-10 md:px-10 md:py-14 lg:grid lg:grid-cols-2 lg:items-center lg:gap-12 lg:py-16 xl:gap-20">
        <div className="opacity-0-start animate-fade-in flex flex-col justify-center lg:max-w-xl xl:max-w-[34rem]">
          <h1 className="font-display text-[2.65rem] font-medium leading-[1.02] tracking-[-0.02em] text-kingdom-ink sm:text-5xl md:text-[3.35rem] lg:text-[3.75rem] xl:text-[4.25rem]">
            Every companion
            <span className="block">deserves an identity.</span>
          </h1>
          <p className="mt-8 max-w-md font-sans text-[14px] leading-[1.85] text-kingdom-ink-muted md:mt-10 md:text-[15px] md:leading-8">
            PetLuma Kingdom Registry provides official companion passports, memorial
            archives, and lifelong identity records.
          </p>
          <div className="mt-10 flex flex-col gap-3 sm:flex-row sm:items-center md:mt-12">
            <RegistryButton href="/create">Create Passport</RegistryButton>
            <RegistryButton href="#registry" variant="secondary">
              Learn More
            </RegistryButton>
          </div>
        </div>

        <div
          className="opacity-0-start animate-fade-in-slow mt-12 flex min-h-[320px] flex-1 items-center justify-center lg:mt-0 lg:min-h-0"
          style={{ animationDelay: "0.15s" }}
        >
          <HeroProductVisual />
        </div>
      </div>
    </section>
  );
}
